...
df_train = pd.read_csv('1114_traning.csv')
df_test = pd.read_csv('1114_test.csv')
...
#X_test = X.iloc[label_test,:]
#y_test = y.iloc[label_test]
...
for i in range(len(X_test)):
    ...
    new_pre_y = model_br.predict(new_point)
    cc.append(list(np.around(new_pre_y)))
gbr_count = 0
for i in range(len(y_test)):
    if (cc[i][0] <= y_test.iloc[i]+0.5)&(cc[i][0] >= y_test.iloc[i]-0.5):
        gbr_count += 1
gbr_count
